...
class PreviewTemplateAction : AnAction(DejavuBundle.message("action.preview.title"), DejavuBundle.message("action.preview.description"), null) {
    override fun actionPerformed(e: AnActionEvent) {
        val project = e.project ?: return
        val virtualFile = e.getData(PlatformDataKeys.VIRTUAL_FILE) ?: return
        
        if (virtualFile.extension != "dejavu" && virtualFile.extension != "dj") {
            Messages.showErrorDialog(project, DejavuBundle.message("error.not.dejavu.file"), DejavuBundle.message("error.title"))
            return
        }
        ...
        try {
            ...
            val dataFilePath = virtualFile.path.replace(Regex("\\.(dejavu|dj)$"), ".data.json")
            ...
            val data = if (dataFile.exists()) {
                try {
                    ...
                    Messages.showErrorDialog(project, DejavuBundle.message("error.invalid.data.file"), DejavuBundle.message("error.title"))
                    return
                }
            } else {
                emptyMap<String, Any>()
            ...
            Messages.showErrorDialog(project, DejavuBundle.message("error.reading.file", e.message ?: "Unknown error"), DejavuBundle.message("error.title"))
        }
    }
    ...
    override fun update(e: AnActionEvent) {
        ...
        e.presentation.isEnabledAndVisible = project != null && virtualFile != null && 
            (virtualFile.extension == "dejavu" || virtualFile.extension == "dj")
    }
}