...
function runTest(testClass, timeoutMs) {
  ...
  const command = isWindows ? `$env:JAVA_HOME='C:\\Users\\28059\\.jdks\\graalvm-jdk-21.0.7'; ./gradlew test --tests "${testClass}" -x prepareTestSandbox` : `${gradleCmd} test --tests "${testClass}" -x prepareTestSandbox`;
    ...
    if (error.message.includes('Timeout')) {
      ...
      directories.forEach(dir => {
        ...
        if (fs.existsSync(fullPath)) {
          ...
          const txtFileNames = new Set(txtFiles.map(file => path.basename(file, '.txt').toLowerCase()));
